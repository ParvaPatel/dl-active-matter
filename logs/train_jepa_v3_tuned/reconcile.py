...
ROOT_DIR = "."
OUTPUT_FILE = "merged_experiment_with_warmup.csv"

# Epoch summary pattern
...
# New best
best_pattern = re.compile(r"New best val loss:\s+([0-9.]+)")

# Warmup / early stopping message
warmup_pattern = re.compile(r"warmup phase", re.IGNORECASE)
    ...
    try:
        ...
        for idx, line in enumerate(lines):
            ...
            for j in range(idx + 1, min(idx + 5, len(lines))):
                ...
                b = best_pattern.search(next_line)
                if b:
                    is_best = True
                    best_val = float(b.group(1))
                    break

                if warmup_pattern.search(next_line):
                    is_warmup = True
